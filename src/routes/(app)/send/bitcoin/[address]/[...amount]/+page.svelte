  ...
  {#if confirmed}
    ...
    <div class="text-center">
      ...
      <div class="relative">
        ...
        <button
          class="absolute right-1/4 top-0 text-secondary p-4"
          on:click={toggleAdjusting}
  ...
  <form method="POST" use:enhance on:submit={toggle}>
    ...
    {#if confirmed && adjusting}
      <div class="relative mt-10 mb-20">
        <div class="absolute top-5 left-0 text-secondary">
          Slower (~40 blocks)
        </div>
        <div class="absolute top-5 right-0 text-secondary">
          Faster (~1 block)
        </div>
