<script>
	...
	let mobileMenu;
	$: logMobileParent(showMobileTip);
	let logMobileParent = () => mobileMenu && console.log(mobileMenu.offsetParent);
		...
		>
			...
			<div
				...
				class="absolute {showMobileTip
					? 'top-[calc(100vh - 100%)]'
					: 'top-[calc(100vh)]'} left-0 bg-white p-6 md:p-0 rounded-t-3xl md:rounded-none transition-all md:transition-none ease-in-out duration-500 md:static text-center space-y-5 z-50"
			>
